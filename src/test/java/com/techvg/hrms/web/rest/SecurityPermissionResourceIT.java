...
import com.techvg.hrms.repository.SecurityPermissionRepository;
import com.techvg.hrms.service.criteria.SecurityPermissionCriteria;
import com.techvg.hrms.service.dto.SecurityPermissionDTO;
import com.techvg.hrms.service.mapper.SecurityPermissionMapper;
...
class SecurityPermissionResourceIT {
    ...
    @Test
    @Transactional
    void getAllSecurityPermissionsByEmployeeIsEqualToSomething() throws Exception {
        Employee employee;
        ...
        em.persist(employee);
        em.flush();
        securityPermission.addEmployee(employee);
        securityPermissionRepository.saveAndFlush(securityPermission);
        Long employeeId = employee.getId();

        // Get all the securityPermissionList where employee equals to employeeId
        defaultSecurityPermissionShouldBeFound("employeeId.equals=" + employeeId);

        // Get all the securityPermissionList where employee equals to (employeeId + 1)
        defaultSecurityPermissionShouldNotBeFound("employeeId.equals=" + (employeeId + 1));
    }
    ...
    @Test
    @Transactional
    void putExistingSecurityPermission() throws Exception {
        // Initialize the database
        securityPermissionRepository.saveAndFlush(securityPermission);
