...
type RestOf<T extends IDependant | NewDependant> = Omit<T, 'lastModified' | 'createdOn'> & {
...
export class DependantService {
  ...
  protected convertDateFromClient<T extends IDependant | NewDependant | PartialUpdateDependant>(dependant: T): RestOf<T> {
    return {
      ...dependant,
      lastModified: dependant.lastModified?.toJSON() ?? null,
      createdOn: dependant.createdOn?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restDependant: RestDependant): IDependant {
    return {
      ...restDependant,
      lastModified: restDependant.lastModified ? dayjs(restDependant.lastModified) : undefined,
      createdOn: restDependant.createdOn ? dayjs(restDependant.createdOn) : undefined,
    };
  }
