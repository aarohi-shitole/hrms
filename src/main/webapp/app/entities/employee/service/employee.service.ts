...
type RestOf<T extends IEmployee | NewEmployee> = Omit<T, 'resetDate' | 'createdOn'> & {
...
export class EmployeeService {
  ...
  protected convertDateFromClient<T extends IEmployee | NewEmployee | PartialUpdateEmployee>(employee: T): RestOf<T> {
    return {
      ...employee,
      resetDate: employee.resetDate?.toJSON() ?? null,
      createdOn: employee.createdOn?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEmployee: RestEmployee): IEmployee {
    return {
      ...restEmployee,
      resetDate: restEmployee.resetDate ? dayjs(restEmployee.resetDate) : undefined,
      createdOn: restEmployee.createdOn ? dayjs(restEmployee.createdOn) : undefined,
    };
  }
