...
type RestOf<T extends IEducation | NewEducation> = Omit<T, 'startDate' | 'endDate' | 'lastModified' | 'createdOn'> & {
...
export class EducationService {
  ...
  protected convertDateFromClient<T extends IEducation | NewEducation | PartialUpdateEducation>(education: T): RestOf<T> {
    return {
      ...
      createdOn: education.createdOn?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEducation: RestEducation): IEducation {
    return {
      ...
      createdOn: restEducation.createdOn ? dayjs(restEducation.createdOn) : undefined,
    };
  }
